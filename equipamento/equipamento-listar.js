...
document.addEventListener("DOMContentLoaded", () => {
  ...
  document.getElementById("btnNovo").addEventListener("click", () => {
    window.location.href = "equipamento-cadastro.html";
  });
  ...
  tbody.addEventListener("click", (ev) => {
    ...
    if (btn.dataset.action === "edit") {
      window.location.href = `equipamento-cadastro.html?id=${id}`;
...
async function carregarEquipamentos() {
  ...
  try {
    const res = await fetch(`${API_URL}/listar/equipamentos`);
    if (!res.ok) {
...
async function deletarEquipamento(idEquipamento) {
  ...
  try {
    const res = await fetch(`${API_URL}/remove/equipamento/${idEquipamento}`, { method: "DELETE" });
    if (!res.ok) {
